 ...
 * @since 1.0.0
 */
class AgendaImporter extends Importer
{
	use Exists;
	...
	/**
	 * Insert a agenda item as post.
	 *
	 * @since 1.0.0
	 */
	public function insert( array $agenda ): int|\WP_Error
	{
		$post_array        = Agenda::to_post_array( $agenda );
		$id_already_exists = $this->exists_id( $agenda, Agenda::POST_TYPE );
	...
	/**
	 * Delete a agenda item as post.
	 *
	 * @since 1.0.0
	 */
	public function delete( array $agenda ): void
	{
		$id_already_exists = $this->exists_id( $agenda, Agenda::POST_TYPE );
	...
	public function add_the_terms( array $post_array, int $post_id ): void
	{
		...
		if ($yfw_settings && ! empty( $yfw_settings['assignedPostTypes'] ) && is_array( $yfw_settings['assignedPostTypes'] )) {
			if (in_array( Agenda::POST_TYPE, $yfw_settings['assignedPostTypes'], true )) {
				$themes = $post_array['meta_input'][ Agenda::POST_TYPE . '_item_themes' ] ?? array();
	...
	/**
	 * Make api call and import the agendas.
	 *
	 * @since 1.0.0
	 */
	public function import_agendas( string $page ): void
	{
		$agenda_items = $this->container->get_agenda_items( $page, self::MAX_BATCH_SIZE );
		...
		if ( ! empty( $agenda_items )) {
			...
			if (count( $agenda_items ) === self::MAX_BATCH_SIZE) {
